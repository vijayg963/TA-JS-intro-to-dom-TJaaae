...
child.innerHTML = `Learning DOM`;

//    - Select the first `li` element inside the `ul` with class `topics` and change the innerText to `all about document`
document.querySelector(
  `.topics`
).firstElementChild.innerHTML = `all about document`;
...
allTopics.forEach((li) => console.log(li.innerHTML));
...
console.log(`Total number of label is ${allLabel.length}`);

// 19. Select all the elements with `id` of `test`
let commonId = document.querySelectorAll(`#test`);
